...
$total_items = 0;
$total_pages = 1;
if ($countRes = $conn->query("SELECT COUNT(*) AS total FROM `mountain`")) {
    $countRow = $countRes->fetch_assoc();
    $total_items = (int)($countRow['total'] ?? 0);
    $total_pages = max(1, (int)ceil($total_items / $limit));
    $countRes->close();
} else {
    error_log('DB error in mountain count: ' . $conn->error);
}
...
<head>
  ...
  <title>Mountain Bikes - AlphaCycles</title>
...
<section id="products" class="pt-8">
  <h2 class="text-center text-3xl font-orbitron font-bold mb-4">Our mountain Collection</h2>

  <?php if ($total_items > 0): ?>
    <div class="page-info">
      Showing <?= (($page - 1) * $limit) + 1 ?> - <?= min($page * $limit, $total_items) ?> of <?= $total_items ?> mountain
...
function updateStockInfo(productId) {
  fetch(`get_stock_info.php?product_id=${productId}&table=mountain`)
    .then(r => r.json())
